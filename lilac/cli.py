...
def new_project(name: str) -> None:
    ...
    (root / "app.py").write_text(textwrap.dedent(f"""
        from lilac import Lilac, Response
    ...
    """))
    (root / "README.md").write_text(f"# {name}\n\nA Lilac app created by `lilac new`\n")
...
def run_app(path: str = ".") -> None:
    ...
    for c in candidates:
        if c.exists():
            ...
            break
    if target is None:
        print("No app.py found in the current folder or examples/. Create one with `lilac new <name>`.")
        return
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="lilac", description="Lilac developer CLI")
    sub = parser.add_subparsers(dest="cmd")

    p_new = sub.add_parser("new", help="Create a new Lilac app scaffold")
    p_new.add_argument("name")

    p_run = sub.add_parser("run", help="Run a Lilac app in the current folder")
